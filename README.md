...
需要在服务器下面的软件。Windows/Linux均按照官方文档安装即可。

 * Node v0.10.30 或者更高版本 Windows直接安装 Linux可以[参照](https://github.com/joyent/node/wiki/Installing-Node.js-via-package-manager)
 * MongoDB v2.6.4或者更高版本  Windows直接安装 Linux可以[参照](http://docs.mongodb.org/manual/administration/install-on-linux/)
 * ImageMagick 6.8.9 或者更高版本

配置如下

 * 如果是Windows系统 Node加入系统Path
 * ImageMagick目录下的可执行文件需要使用。也加入系统path。
 * Node注册为系统服务 [Windows参照](http://www.oschina.net/question/12_18694?sort=time) linux直接将sh放到etc/init.d即可。
 * Mongo注册为系统服务 按照Node一样的方法接口 或者也可以[参照](http://blog.csdn.net/chaijunkun/article/details/7227967)  linux同上。
 * Node注册为系统服务时，注意配置Path加入ImageMagick的可执行文件所在目录。避免调用可执行文件失败。
 
Windows bat典型配置：
```
@echo off
d:
cd d:\ECXService\ImageService\src
set PATH="C:\Program Files\ImageMagick-6.8.9-Q16";"C:\Program Files (x86)\ImageMagick-6.8.9-Q16";%PATH%;
node app.js
```
...
```
 mongodump -h 127.0.0.1 --port 27017 -u yusure -p yusure -d ECXService -o ./dump
```